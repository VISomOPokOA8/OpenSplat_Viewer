...
int main(int argc, char *argv[]){
    ...
    options.add_options()
        ...
        ("n,num-iters", "Number of iterations to run", cxxopts::value<int>()->default_value("2000"))
    ...
    try{
        ...
        int imageSize = -1;
        // 在循环外部，创建一个用于渲染点云的相机
        Camera pointCloudCam;
        // 初始化 pointCloudCam 的参数，使其能捕捉点云的合适视角
        // 比如你可以根据需要设定一个固定的视角，或者自动计算一个最佳的视角
        // 以下仅为示例，你需要根据实际需求设置相机参数
        pointCloudCam.fx = 800.0f;
        pointCloudCam.fy = 800.0f;
        pointCloudCam.cx = 400.0f;
        pointCloudCam.cy = 300.0f;
        pointCloudCam.height = 600;
        pointCloudCam.width = 800;
        pointCloudCam.camToWorld = torch::eye(4); // 初始化为单位矩阵（表示简单的视角）

        // 添加相机移动的速度
        float cameraMoveSpeed = 0.1f;  // 调整这个值来控制移动的速度

        cv::Mat finalCombinedImage;
        ...
        for (size_t step = 1; step <= numIters; step++) {
            ...
            if (saveEvery > 0 && step % saveEvery == 0) {
                ...
                model.save((p.replace_filename(fs::path(p.stem().string() + p.extension().string())).string()));
            }
            ...
            // 渲染主场景的 RGB 图像
            cv::Mat displayImage = tensorToImage(rgb.detach().cpu());
            cv::cvtColor(displayImage, displayImage, cv::COLOR_RGB2BGR);

            // 使用相同的渲染方式渲染点云的视角
            torch::Tensor pointCloudRGB = model.forward(pointCloudCam, step);
            cv::Mat pointCloudImage = tensorToImage(pointCloudRGB.detach().cpu());
            cv::cvtColor(pointCloudImage, pointCloudImage, cv::COLOR_RGB2BGR);

            // 调整两个图像的大小，使它们一致
            if (displayImage.size() != pointCloudImage.size()) {
                cv::resize(pointCloudImage, pointCloudImage, displayImage.size());
            }

            // 合并两个图像（水平合并）
            cv::Mat combinedImage;
            cv::hconcat(displayImage, pointCloudImage, combinedImage);

            // 显示合并后的图像
            cv::imshow("Combined Rendered View", combinedImage);
            ...
            if (key == 27) { // 按下 'Esc' 键退出
                ...
                // 向前移动相机（减小 Z 轴值）
                pointCloudCam.camToWorld.index_put_({2, 3}, pointCloudCam.camToWorld.index({2, 3}).item<float>() - cameraMoveSpeed);
            } else if (key == 's' || key == 'S') {
                // 向后移动相机（增大 Z 轴值）
                pointCloudCam.camToWorld.index_put_({2, 3}, pointCloudCam.camToWorld.index({2, 3}).item<float>() + cameraMoveSpeed);
            }

            // 储存最后一帧的合并图像，用于建模结束后显示
            if (step == 1 || step == numIters) {
                finalCombinedImage = combinedImage.clone();
            }
        }
        ...
        while (true) {
            // 显示合并后的图像
            cv::imshow("Combined Rendered View", finalCombinedImage);

            // 处理键盘输入，退出程序
            int key = cv::waitKey(1);
            if (key == 27) { // 按下 'Esc' 键退出
                std::cout << "Closing window and ending program..." << std::endl;
                break;
            }
        }
